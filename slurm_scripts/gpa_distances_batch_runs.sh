...
#
# What it does:
#   Scans PANAROO_RUN_ROOT (set below) for each immediate
#   subdirectory that contains gene_presence_absence.Rtab, and runs the same
#   analysis as a single-run job on every such folder in parallel (--workers).
#   Writes one compiled TSV (gpa_reference_batch_summary_<timestamp>.tsv) under
#   --output-dir (default: .../processed/pangenome_analysis).
#
