...
def process_parquet_file(filepath: str) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        'filename': [filename] * len(genome_ids)
    })
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Create genome_id to parquet file mapping from ESMC embeddings'
    )
    parser.add_argument(
        '--data-dir',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Output CSV path (default: genome_to_parquet_file_mapping.csv in data directory)'
    )
    parser.add_argument(
        '--max-files',
        type=int,
    ...
    with Pool(processes=args.processes) as pool:
        results = pool.map(process_parquet_file, file_list)
    ...
    if args.output is None:
        output_path = os.path.join(args.data_dir, 'genome_to_parquet_file_mapping.csv')
    else:
