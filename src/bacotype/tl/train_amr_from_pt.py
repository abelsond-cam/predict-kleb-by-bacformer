...
def run(
    ...
    """Fine-tune Bacformer on AMR data using .pt files directly."""
    ...
    if not ast_sheet_path:
        raise ValueError("ast_sheet_path must be provided (binary_ast_with_split_pt.csv)")
    ...
    if "train_val_eval" not in ast_df.columns:
        raise ValueError(
            "AST sheet must contain 'train_val_eval' column. Run prepare_klebsiella_ast_splits_as_pt.py first."
        )
    ...
    # Build file lists: .pt files named {sample_id}_with_ast.pt
    ...
    def build_file_list(split_name: str) -> list[Path]:
        ...
        for sid in ids:
            ...
            if path.exists():
                ...
                print(f"WARNING: Expected .pt file missing for Sample {sid}: {path}")
        return files
    ...
    if n_samples == 10:
        ...
        print("Full set mode: loading .pt files via PTFileDataset")
    ...
    if not train_files:
        raise RuntimeError(
            f"No .pt files found for training (check AST sheet, drug column, and {train_data_dir})"
        )

    print(f"Number of train files (samples with '{drug}'): {len(train_files)}")
    print(f"Number of validation files: {len(val_files)}")

    train_dataset = PTFileDataset(train_files, drug=drug)
    val_dataset = PTFileDataset(val_files, drug=drug)
...
class ArgumentParser(Tap):
    """Argument parser for training Bacformer from .pt files."""

    def __init__(self):
        super().__init__(underscores_to_dashes=True)

    model_name_or_path: str = "macwiatrak/bacformer-large-masked-MAG"
    train_data_dir: str = "/home/dca36/rds/rds-floto-bacterial-4k08a2yyQLw/david/processed/ast_training_pt/train"
    val_data_dir: str = "/home/dca36/rds/rds-floto-bacterial-4k08a2yyQLw/david/processed/ast_training_pt/validate"
    ...
    logging_steps: int = 10
    n_samples: int = 10000
    ast_sheet_path: str = "/home/dca36/rds/rds-floto-bacterial-4k08a2yyQLw/david/processed/binary_ast_with_split_pt.csv"
    seed: int = 1
...
if __name__ == "__main__":
    ...
    print("Running finetuning from .pt files")
